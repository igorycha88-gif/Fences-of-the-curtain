...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await materialsService.getFenceMaterials(params);

    return NextResponse.json(result);
  } catch (error) {
    console.error('Error fetching fence materials:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const material = await materialsService.createFenceMaterial(body, session.user.id);

    return NextResponse.json(material, { status: 201 });
  } catch (error) {
    console.error('Error creating fence material:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
