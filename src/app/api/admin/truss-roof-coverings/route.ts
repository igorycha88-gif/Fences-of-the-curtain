...
export async function GET(request: NextRequest) {
  try {
    ...
    const [coverings, total] = await Promise.all([
      prisma.trussRoofCovering.findMany({
        where,
        orderBy: [{ priority: 'asc' }, { name: 'asc' }],
        skip: (page - 1) * pageSize,
        take: pageSize,
      }),
      prisma.trussRoofCovering.count({ where }),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = trussRoofCoveringCreateSchema.parse(body);
    ...
    const maxPriority = await prisma.trussRoofCovering.aggregate({
      _max: { priority: true },
    });
    data.priority = (maxPriority._max.priority ?? -1) + 1;

    const covering = await prisma.trussRoofCovering.create({ data });
    return NextResponse.json({ id: covering.id }, { status: 201 });
  } catch (error: any) {
    if (error instanceof ZodError) {
      console.error('[TRUSS-ROOF-COVERINGS POST] Validation failed:', JSON.stringify(error.errors));
