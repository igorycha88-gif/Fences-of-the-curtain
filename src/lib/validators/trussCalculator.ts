...
export const trussCalculationRequestSchema = z.object({
  ...
  ridgeHeight: z.number().min(500).max(6000),
  wallHeight: z.number().min(500).max(6000).optional(),
...
export const trussProfileCreateSchema = z.object({
  ...
  purchasePricePerMeter: z.number().nonnegative().nullable().optional(),
  retailPricePerUnit: z.number().nonnegative().default(0),
  purchasePricePerUnit: z.number().nonnegative().nullable().optional(),
...
export const trussRoofCoveringCreateSchema = z.object({
  ...
  purchasePricePerSqm: z.number().positive().nullable().optional(),
  image: z.string().optional(),
  isActive: z.boolean().default(true),
  priority: z.number().default(0),
  validFrom: z.string().optional().nullable(),
