...
export async function parseYandexSerp(page: Page): Promise<ParsedSerp> {
  ...
  const results: SerpResult[] = await page.evaluate(() => {
    ...
    const serpItems = document.querySelectorAll<HTMLElement>(
      'li.serp-item, [class*="serp-item"], div.Organic'
    );
    ...
    for (const item of serpItems) {
      ...
      const link =
        ...
        item.querySelector<HTMLAnchorElement>('.OrganicTitle a[href^="http"]') ||
      ...
      const title =
        ...
        item.querySelector('.OrganicTitle')?.textContent?.trim() ||
        '';

      let snippet: string | undefined;
      const snippetEl = item.querySelector(
        '.TextContainer, .OrganicTextContent-Text, [class*="OrganicText"], .Typo, .OrganicText'
      );
      if (snippetEl) {
