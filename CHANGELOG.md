...
### Добавлено
- feat(seo): пакет SEO-страниц — посадочные «Забор на 6 сотках», «Навесы на зиму от снега», «Навесы под ключ — цены», «Сколько погонных метров в сотках», «Сколько стоит забор — сравнение»; компоненты LeadForm + SeasonCountdown; +5 URL в sitemap, анонс в /blog (2372483)
- feat(legal): реквизиты ИП на сайте — страница /rekvizity, юр.строка в футерах, legalName/taxID в Organization JSON-LD, +14 автотестов (c9b7beb)
- feat(ui): мобильная кнопка «Позвонить» в hero главной + трекинг phone_click (6895d8b)

### Исправлено
- fix(calculator): сетка-рабица — лаги не рассчитываются (не нужны конструктивно), скрыт селектор «Количество лаг», +3 автотеста (babfc9c)
- fix(ui): фон шапки при скролле — glass-header вместо glass, фикс «жёлтого пятна» (1353d87)
- fix(ci): Telegram-уведомления деплоя — приоритет TELEGRAM_API_IP над мёртвым прокси (bb9c1f8)
...
### Добавлено
- feat(seo): реализация ЧТЗ v3 — Шатура на главной, размерные посадочные навесов, хаб «забор по соткам», «Монтаж за 1 день» (bc747d0)

### Исправлено
- fix(telegram): retry-логика отправки уведомлений в analytics-notifier (3 попытки, backoff, таймаут 8s, обработка 429/5xx/network) — фикс потери уведомлений (bb5cf1b)
- fix(ci): deploy-vps.sh healthcheck timeout 90s → 180s (afc5dc8)

## [1.18.2] - 2026-08-29

### Исправлено
- fix(metrics): переименование `business_visitor_geo_24h{city}` → `business_geo_visitors_24h{city}` в `/metrics/tracking` для соответствия контракту `ЧТЗ_Централизованный_мониторинг.md` (дашборд site-business.json настроен на новое имя; ранее панель «Гео посетителей» была пуста из-за расхождения в порядке слов) (4f33181)

### Документация
- docs: ЧТЗ по фиксу интермитентных 502 ошибок на проде (nginx → IPv6 loopback) — выполнено 25.08.2026

## [1.18.1] - 2026-08-29

### Добавлено
- feat(metrics): блоки «Клики по телефону» (KPI + по дням) и «Гео посетителей» на дашборде бизнес-метрик; листы в XLSX-экспорте
- feat(metrics): метрика business_visitor_geo_24h{city} в /metrics/tracking; агрегация analytics:geo:daily (город по IP на старте сессии)
- feat(metrics): трекинг кликов по телефону на всех публичных tel:-ссылках (гео-страницы городов, «Навесы под ключ», AboutCTA, портфолио)

## [1.18.0] - 2026-08-29

### Добавлено
- feat(admin): полные бизнес-метрики — дашборд с KPI-трендами, воронкой, таймлайном, причинами отмен, менеджерами и экспортом XLSX (ЧТЗ) (2a4a735)

## [1.17.0] - 2026-08-25

### Добавлено
- feat(seo): гео-страницы Подмосковья — 32 города + 3 хаба направлений + страница «Навесы под ключ» (ЧТЗ) (80e1a2a)

## [1.16.0] - 2026-08-24

### Добавлено
- feat(monitoring): подключение к централизованному мониторингу (d730eac)

### Изменено
- chore(opencode): подключить AGENTS.md и скилл zabor-naves в конфиг проекта (7a9631e)

## [1.15.1] - 2026-08-21

### Исправлено
- fix(seo): убран Crawl-delay: 1 из robots.txt для Яндекса — искусственно замедлял обход робота (ЧТЗ_SEO_Фиксы_Яндекс)
- fix(seo, infra): починено www-зеркало — сертификат расширен SAN www.zabor-i-naves.ru + 301 редирект на основное зеркало (VPS nginx)
