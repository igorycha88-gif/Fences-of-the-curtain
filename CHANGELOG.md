...
### Исправлено
- fix(metrics): переименование `business_visitor_geo_24h{city}` → `business_geo_visitors_24h{city}` в `/metrics/tracking` для соответствия контракту `ЧТЗ_Централизованный_мониторинг.md` (дашборд site-business.json настроен на новое имя; ранее панель «Гео посетителей» была пуста из-за расхождения в порядке слов)
