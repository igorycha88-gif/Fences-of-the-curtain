...
async function main() {
  ...
  const postTypes = await Promise.all([
    ...
    prisma.postType.upsert({
      where: { id: 'post-mesh-48x1.5' },
      update: {},
      create: {
        id: 'post-mesh-48x1.5',
        name: 'Столб для сетки 48x1.5',
        description: 'Столб для забора из сетки-рабицы',
        sectionWidth: 48,
        sectionHeight: 48,
        wallThickness: 1.5,
        pricePerMeter: 200,
    ...
    prisma.postType.upsert({
      where: { id: 'post-mesh-60x2.0' },
      update: {},
      create: {
        id: 'post-mesh-60x2.0',
        name: 'Столб для сетки 60x2.0',
        description: 'Усиленный столб для забора из сетки-рабицы',
        sectionWidth: 60,
        sectionHeight: 60,
        wallThickness: 2.0,
        pricePerMeter: 300,
