  ...
  try {
    ...
    if (workRelations.length === 0) {
      ...
      for (const rel of workRelations) {
        const panelId = rel.referenceId;
        const panel = await prisma.panel3D.findUnique({
          where: { id: panelId },
          select: { name: true, active: true }
        });
        
        const panelName = panel ? panel.name : 'Неизвестно';
        const panelActive = panel ? (panel.active ? '✅' : '❌') : '-';
        ...
        console.log(`   - Работа "${workName}" связана с панелью "${panelName}" (ID: ${panelId})`);
        console.log(`     Панель активна: ${panelActive}, Работа активна: ${workActive}, В кальк: ${workUseInCalc}`);
      }
    }
