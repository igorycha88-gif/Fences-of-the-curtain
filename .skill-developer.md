...
export async function POST(req: NextRequest) {
  ...
  logger.info('API request', { method: 'POST', path: '/api/orders', userId });
...
- **НЕ логировать секреты** (пароли, токены, ключи)
...
describe('OrdersService — logging', () => {
  ...
  it('should log on successful order creation', async () => {
    ...
    await ordersService.createOrder({ clientName: 'Test', phone: '+79991234567' });
  ...
  it('should log error on failed order creation', async () => {
    ...
    await expect(
      ordersService.createOrder({ clientName: 'Test', phone: '+79991234567' })
